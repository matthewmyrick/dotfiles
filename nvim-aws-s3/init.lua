-- nvim-aws-s3: S3 Browser for Neovim
-- Tree view browser for AWS S3 buckets
...
-- Startup: Show region picker (Telescope), then open drawer with buckets
      ...
      pickers.new({}, {
        ...
        attach_mappings = function(prompt_bufnr)
          actions.select_default:replace(function()
            ...
            if selection then
              -- Set region
              require("s3").set_region(selection[1])

              -- Open drawer and load buckets
              local drawer = require("s3.drawer")
              drawer.open()
              drawer.load_buckets()
            end
          end)
...
vim.keymap.set("n", "<leader>so", function()
  require("s3.drawer").open()
end, { desc = "Open S3 drawer" })

vim.keymap.set("n", "<leader>sc", function()
  require("s3.drawer").close()
end, { desc = "Close S3 drawer" })

vim.keymap.set("n", "<leader>sr", function()
  require("s3.drawer").load_buckets()
end, { desc = "Reload S3 buckets" })
...
-- Metadata command to view file metadata
vim.api.nvim_create_user_command("S3Meta", function()
  require("s3.drawer").show_current_metadata()
end, { desc = "Show S3 file metadata" })

vim.keymap.set("n", "<leader>m", "<cmd>S3Meta<CR>", { desc = "Show S3 metadata" })
